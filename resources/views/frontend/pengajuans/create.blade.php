        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('frontend.pengajuans.store') }}" enctype="multipart/form-data">
                            @method('POST')
                            @csrf
                            <div class="form-group">
                                <label class="required"
                                    for="program_id">{{ trans('cruds.pengajuan.fields.program') }}</label>
                                <select class="form-control select2" name="program_id" id="program_id" required>
                                    @foreach ($programs as $id => $entry)
                                        @if (!$mahasiswa->hasSubmittedProgram($id))
                                            <option value="{{ $id }}"
                                                {{ old('program_id') == $id ? 'selected' : '' }}>
                                                {{ $entry }}
                                            </option>
                                        @endif
                                    @endforeach
                            ...
                            <div class="form-group">
                                <label class="required"
                                    for="semester">{{ trans('cruds.pengajuan.fields.semester') }}</label>
