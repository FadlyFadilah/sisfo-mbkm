                            ...
                            <tbody>
                                @foreach($laporans as $key => $laporan)
                                    <tr data-entry-id="{{ $laporan->id }}">
                                        ...
                                        <td>
                                            @can('laporan_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.laporans.show', $laporan->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('laporan_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.laporans.edit', $laporan->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('laporan_delete')
                                                <form action="{{ route('frontend.laporans.destroy', $laporan->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('laporan_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.laporans.massDestroy') }}",
